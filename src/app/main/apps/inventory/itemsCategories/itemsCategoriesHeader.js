...
function ItemsCategoriesHeader(props) {
  ...
  const handleActiveStatusChange = async (event) => {
    try {
      ...
      dispatch(setItemsCategoriesActiveStatus(event))
    } catch (error) {
      console.log(error)
    }
  }
  ...
  const handleAddItemsCategory = async () => {
    try {
      if (user.data.country !== "") {
        navigate("/apps/inventory/itemscategory/new")
      } else {
    ...
    <div className="flex flex-col sm:flex-row space-y-16 sm:space-y-0 flex-1 w-full items-center justify-between py-32 px-24 md:px-32">
      ...
      <div className="flex flex-col w-full sm:w-auto sm:flex-row space-y-16 sm:space-y-0 flex-1 items-center justify-end space-x-8">
        ...
        {/* <motion.div
          initial={{ opacity: 0, x: 20 }}
          animate={{ opacity: 1, x: 0, transition: { delay: 0.2 } }}
          className='flex items-center rounded-full shadow-0'
          style={{ cursor: "pointer" }}
        >
          <Button className='rounded-circle' onClick={handleOnRefresh}>
        ...
        >
          ...
          <Input
            ...
            onChange={(ev) => dispatch(setItemsCategoriesSearchText(ev))}
          />
        </Paper>
        <motion.div
          initial={{ opacity: 0, x: 20 }}
        ...
        >
          <Button
            className=""
            onClick={handleAddItemsCategory}
            variant="contained"
            color="secondary"
...
export default ItemsCategoriesHeader;
