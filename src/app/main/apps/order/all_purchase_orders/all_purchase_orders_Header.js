...
function AllPurchaseOrdersHeader(props) {
  ...
  const handleActiveStatusChange = async (event) => {
    try {
      ...
      dispatch(setpurchaseordersActiveStatus(event))
    } catch (error) {
      console.log(error)
    }
  }
      ...
      <div className="flex flex-col w-full sm:w-auto sm:flex-row space-y-16 sm:space-y-0 flex-1 items-center justify-end space-x-8">
        ...
        >
          ...
          <Input
            ...
            onChange={(ev) => dispatch(setpurchaseordersSearchText(ev))}
          />
        </Paper>
      </div>
    </div>
...
export default AllPurchaseOrdersHeader;
